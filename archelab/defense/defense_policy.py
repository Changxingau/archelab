...
def apply_defense_to_tool_event(
    ...
    is_write_like = "write" in tool_name_lower or "save" in tool_name_lower
    ...
    if any(marker in path for marker in config.protected_path_markers):
        if rng.random() < config.block_unauthorized_write_prob:
            blocked = True
            actions.append(
                DefenseAction(
                    rule_id="block_unauthorized_write",
                    reason=f"Blocked write to protected path: {path}",
                    applied=True,
                )
            )
        else:
